...
def sequence_embed(embed, xs):
    # xsはIDのリストのリスト
    # [[1,56,1,1103,12], [20,11004,35], [22,51]]みたいな
    # x_len = [5,3,2]
    x_len = [len(x) for x in xs]
    # x_section = [5,8,10]
    x_section = numpy.cumsum(x_len[:-1])
    # F.concat(xs, axis=0)で1次元の配列になる
    # おそらくembed_idが1次元配列での入力しか受け付けないためこういった処理をしていると思われる
...
class Seq2seq(chainer.Chain):
    ...
    def __call__(self, xs, ys):
        ...
        # target言語の最初の入力としてeosをconcatする
        ys_in = [F.concat([eos, y], axis=0) for y in ys]
        # target言語の翻訳終了のシンボルとしてeosをconcat
        ...
        # Lossの計算
        # reduce=noにしているのはまずは翻訳文毎にlossを計算するためと思われる
        # そのあとでsumしてミニバッチ数で割る
    ...
    def translate(self, xs, max_length=100):
        ...
        with chainer.no_backprop_mode(), chainer.using_config('train', False):
            ...
            # 長さがbatchでEOS(=1)で埋められた1次元の配列。iはデータ型でint
            ys = self.xp.full(batch, EOS, 'i')
            result = []
            # max_lengthまでは強制手に回すので、EOSが出力されたら
            for i in range(max_length):
                ...
                wy = self.W(cys)
                # ysにはバッチの各データの出力候補単語のIDが含まれている
                # バッチが5件なら、[10021,4220, 2053, 211, 50025]といった形
                ys = self.xp.argmax(wy.data, axis=1).astype('i')
                result.append(ys)

        # 行列をスタックして転置することで行が1つの翻訳文(のID列)になる
...
def load_data(vocabulary, path):
    ...
    with open(path) as f:
        for line in bar(f, max_value=n_lines):
            ...
            # dict.get(w, UNK)は辞書にwがなかったらUNKのid(=0)を返す
            # ファイルの1行に含まれる単語をIDに変換したnumpyのarray
...
def main():
    ...
    # giga-fren.preprocess.en
    parser.add_argument('SOURCE', help='source sentence list')
    # giga-fren.preprocess.fr
    ...
    # sourceとtargetのタプル。sとtはIDの格納されたnumpy array
    train_data = [(s, t)
    ...
    # validation sourceは学習させながら途中での翻訳結果を確認したりBLEUスコアを計算するのに使用する
    if args.validation_source and args.validation_target:
        ...
        # 関数をTrainerのExtension化する
        # 今までExtensionを継承したクラスを作って__call__でTrainerを受け取るメソッドを書いてたけど
        # trainerを受け取るメソッド書いてデコレーターをつけるだけ
        @chainer.training.make_extension(trigger=(200, 'iteration'))
        def translate(trainer):
            source, target = test_data[numpy.random.choice(len(test_data))]
            # !? modelは勝手に参照してるけどtrainerの変数から取得しにいかなくていいのか？
